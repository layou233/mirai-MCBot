...
//                    val whom: String = this.sender.toString().substring(7, (this.sender.toString().length - 1))
                        val saying = this.message.contentToString().replace("\\s", "")
//                    logger.info { saying }
                        if (saying.length > 1) {
                        ...
                        if (saying.length > 4) {
                            ...
                            else if (saying.substring(0, 4) == "skin") {
                        ...
                        if (saying.toLowerCase(ROOT) == "status") this.group.sendMessage(buildMessageChain {
                            +this@subscribeAlways.message.quote()
                            +" 机器人状态：运行中……"
                        })
                    }
                    is FriendMessageEvent -> {
                        val saying = this.message.contentToString().replace("\\s", "")
                        if (saying.length > 2) {
                            if (saying.toLowerCase(ROOT).substring(0, 2) == "chaq") {
                                val qqNumber: String = saying.substring(2)
                                this.sender.sendMessage(buildMessageChain {
                                    "查询 $qqNumber 结果：\n手机绑定：${
                                        getQBounding(qqNumber)
                                    }"
                                })
                            }
                        }
                        if (saying.toLowerCase(ROOT) == "status") this.sender.sendMessage("机器人状态：运行中……")
                    }
                    is GroupTempMessageEvent -> this.sender.sendMessage("为防止部分离奇问题出现，请先添加好友再使用功能！")
                }
            } catch (e: IOException) {
                when (this) {
                    is GroupMessageEvent -> this.group.sendMessage(buildMessageChain {
                        +this@subscribeAlways.message.quote()
                        +" 网络请求出现故障，请重试？"
                    })
                    is FriendMessageEvent -> this.sender.sendMessage("网络请求出现故障，请重试？")
                }
            }
        }
    }
